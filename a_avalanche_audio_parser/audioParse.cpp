...
vector<float> convert_fft_to_vector(cvec_t input) {
  ...
  vector<float> output_vec(input.length-1);
...
vector<float> create_single_fft_vector(vector<int> input_vector, int PSDPOWER){
  ...
  */
  ...
  assert(log2(magnitudes.size()) == int(log2(magnitudes.size()))); //Ensure that the waveformVecs size is a power of 2
...
int main()
{
    ...
    //2^14 = 16384 = ~8000 hz
    const int PSDPOWER = 10;

    //16384 =2^14 produces ~8150 ffts bins
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    timeWavePower.writeTWP("output.twp");
    ...
    char* drumPath;
    drumPath = "../spleeter_tracks/input4stem/input/drums.wav";

    //create object to hold a bunch of important information
    //see wavSoundData.cpp file to implementation
    soundData drum_track;
    drum_track.parse_header_and_body(drumPath);
    vector<int> drumform = drum_track.retreiveWaveChannel(); //*might* have to allocate on heap

    //initialize a vector to write ffts to with proper size
    vector<float> drum_fft_vec(samplesPerPSD/2);

    vector<int> drum_tempWaveData(samplesPerPSD);

    twp drum_timeWavePower;
    //Set the paddings so that the written file knows how to sync video to PSDs
    drum_timeWavePower.setFPS(FRAMESPERSEC);
    drum_timeWavePower.setSampFreq(drum_track.wavHeader.SamplesPerSec);
    drum_timeWavePower.setPaddings(startFrameDelay, endFrameHalt);
    drum_timeWavePower.addWavetoTWP(drumform);

    for (int i = startFrameDelay; i<numFramesInSound-endFrameHalt; i++) {
      targetSample = i*samplesPerFrame;
      copy(drumform.begin()+targetSample-(samplesPerPSD/2 - 1), drumform.begin()+targetSample+samplesPerPSD/2, drum_tempWaveData.begin());
      drum_fft_vec = create_single_fft_vector(drum_tempWaveData, PSDPOWER);
      drum_timeWavePower.addPSDtoTWP(drum_fft_vec);
    }

    drum_timeWavePower.writeTWP("drum_out.twp");
