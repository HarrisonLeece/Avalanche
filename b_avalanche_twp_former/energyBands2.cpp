...
class energyBands
{
  public:
    ...
    int numBands = 16; // How many energy bands to divide the fft into
    int energyHistory = 32; //How many frames of energy data should be kept for the moving average of band specific engergy
    double variance = 11; //The multiple by which a currentEnergy must exceed energyMem in order to be known as a beat
...
vector<int> energyBands::detectBeat()
{
  ...
  for (int band = 0; band < numBands; band++)
  {
    ...
    if (currentEnergy[band] > energyMem[band] * variance && energyMem[band] != 0)
    {
      ...
      energyBandVec[band].push_back(energyMem[band]);//Replace with the average energy for this band
      if (isDiagnostic)
